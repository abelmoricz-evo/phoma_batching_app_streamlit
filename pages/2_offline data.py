...
def show_offline_data():
    st.header("avaliable offline data", divider=True)
    engine = create_engine(f"postgresql://postgres:DsRdPPJtetGDiMFypvHpUJUKAwEXfoSG@junction.proxy.rlwy.net:19704/PHOMA")
    df = pd.read_sql(f""" SELECT * FROM "OFFLINE" """, con=engine)
    st.write(f"number of OFFLINE data rows: {len(df.index)}")
    st.dataframe(df)

if __name__ == "__main__":
    st.header("offline data", divider=True)
